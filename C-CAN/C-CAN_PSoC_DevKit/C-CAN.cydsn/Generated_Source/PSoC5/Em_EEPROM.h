...
#if (CY_PSOC3)
    #define Em_EEPROM_CODE_ADDR_OFFSET   (0xff0000u)
    #define Em_EEPROM_CODE_ADDR_END      (Em_EEPROM_CODE_ADDR_OFFSET + (Em_EEPROM_FLASH_SIZE - 1u))
    #define Em_EEPROM_CODE_ADDR_MASK     (0xffffu)
#endif /* (CY_PSOC3) */
...
*        Function Prototypes
***************************************/
void Em_EEPROM_Start(void) ;
void Em_EEPROM_Stop(void) ;
#if (CY_PSOC3)
    cystatus Em_EEPROM_Write(const uint8 srcBuf[], const uint8 eepromPtr[],
                                                uint16 byteCount) ;
#else
