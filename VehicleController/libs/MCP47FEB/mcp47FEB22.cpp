...
uint8_t mcp47FEB22::analogWrite(uint16_t value1, uint16_t value2) {
  _values[0] = (value1 & 0x0FFF);
  _values[1] = (value2 & 0x0FFF);
  _return = 0;
...
void mcp47FEB22::getStatus()
{
  ...
  for (uint8_t channel=0; channel <= 1; channel++) {
      //DAC Outputs
      Wire1.beginTransmission(_dev_address);
      Wire1.write(READ | channel << 3);  //Bits 7-2 = address (0x0FFF Vol, 0x1FFF non-Vol), Bits 2-1 = command bits (11 = read) Bit 0=x.
      Wire1.endTransmission(false);
      Wire1.requestFrom((int)_dev_address, 2);
      int hiByte = Wire1.read();
      int loByte = Wire1.read();
  ...
  for (uint8_t channel=0; channel <= 1; channel++) {
      //DAC Outputs
      Wire1.beginTransmission(_dev_address);
      Wire1.write(0x80 | (READ | channel << 3));  //Bits 7-2 = address (0x0FFF Vol, 0x1FFF non-Vol), Bits 2-1 = command bits (11 = read) Bit 0=x.
      Wire1.endTransmission(false);
      Wire1.requestFrom((int)_dev_address, 2);
      int hiByte = Wire1.read();
      int loByte = Wire1.read();
   ...
   //Vref
  Wire1.beginTransmission(_dev_address);
  Wire1.write(0x80 | (READ | VREF_REG << 3));
  Wire1.endTransmission(false);
  Wire1.requestFrom((int)_dev_address, 2);
  while(Wire1.available())
  {
  ...
  //Power-Down
  Wire1.beginTransmission(_dev_address);
  Wire1.write(0x80 | (READ | PD_REG << 3));
  Wire1.endTransmission(false);
  Wire1.requestFrom((int)_dev_address, 2);
  while(Wire1.available())
  {
  ...
  //Gain
  Wire1.beginTransmission(_dev_address);
  Wire1.write(0x80 | (READ | GAIN_REG << 3));
  Wire1.endTransmission(false);
  Wire1.requestFrom((int)_dev_address, 2);
  while(Wire1.available())
  {
...
uint8_t mcp47FEB22::writeVref() {
  Wire1.beginTransmission(_dev_address);
  Wire1.write(WRITE | VREF_REG << 3);
  Wire1.write(0x00);
  Wire1.write(_intVref[0] | _intVref[1]);
  return Wire1.endTransmission();
}
/*
...
uint8_t mcp47FEB22::writeGain() {
  Wire1.beginTransmission(_dev_address);
  Wire1.write(WRITE | GAIN_REG << 3);
  Wire1.write(_gain[0] | _gain[1]);
  Wire1.write(0x00);
  return Wire1.endTransmission();
}
/*
...
uint8_t mcp47FEB22::writePowerDown() {
  Wire1.beginTransmission(_dev_address);
  Wire1.write(WRITE | PD_REG << 3);
  Wire1.write(0x00);
  Wire1.write(_powerDown[0] | _powerDown[1]);
  return Wire1.endTransmission();
}
/*
...
uint8_t mcp47FEB22::_simpleCommand(byte simpleCommand) {
  Wire1.beginTransmission(GENERALCALL);
  Wire1.write(simpleCommand);
  return Wire1.endTransmission();
}